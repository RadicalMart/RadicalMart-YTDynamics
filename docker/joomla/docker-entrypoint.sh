...
radicalmart_download_archive() {
    local VERSION_ID=$1
    local API_TOKEN=$2

    local BASE_URL="https://sovmart.ru/api/v1/projects/download/3"
    local OUTPUT_DIR="/var/www/html/tmp"
    local OUTPUT_FILE="pkg_radicalmart.zip"
    local FULL_PATH="${OUTPUT_DIR}/${OUTPUT_FILE}"
    local URL="${BASE_URL}?version_id=${VERSION_ID}"

    if ! mkdir -p "${OUTPUT_DIR}"; then
        echo "Ошибка: Не удалось создать директорию ${OUTPUT_DIR}. Проверьте права доступа."
        return 1
    fi

    echo "Начинаю загрузку из: ${URL}"
    echo "Сохраняю в: ${FULL_PATH}"
...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
    ...
    if [ -d installation ] && [ -e installation/joomla.php ] && joomla_can_auto_deploy; then
        ...
        if php installation/joomla.php install "${installJoomlaArgs[@]}"; then
            ...
            if [[ -n "${SOVMART_TOKEN}" && ${#SOVMART_TOKEN} -gt 10 && \
                  -n "${RADICAL_MART_VERSION_ID}" && "${RADICAL_MART_VERSION_ID}" =~ ^[0-9]+$ && \
                  "${RADICAL_MART_VERSION_ID}" -gt 0 ]]; then
                radicalmart_download_archive "${RADICAL_MART_VERSION_ID}" "${SOVMART_TOKEN}"
             fi
